...
async function getFastPP(conn, userId) {
  if (ppCache.has(userId)) return ppCache.get(userId)
  const url = await conn.profilePictureUrl(userId, 'image')
    .catch(() => 'https://raw.githubusercontent.com/The-King-Destroy/Adiciones/main/Contenido/1745522645448.jpeg')
  ppCache.set(userId, url)
  return url
}

async function generarImagenFast({ title, desc, avatar, background, key }) {
...
> • .˚ *${texto}*`
  ...
  const key = `${userId}-${tipo}-${groupMetadata.id}`

  const image = await generarImagenFast({
    title: tipo === 'welcome' ? '🌹 Bienvenido/a al grupo' : '🌳 Hasta pronto',
    desc: tipo === 'welcome' ? '¡Disfruta tu estadía!' : '¡Te esperamos de nuevo!',
    avatar: pp,
    background: tipo === 'welcome'
...
handler.before = async function (m, { conn, groupMetadata }) {
  try {
    ...
    if (m.messageStubType === WAMessageStubType.GROUP_PARTICIPANT_ADD) {
      const { image, caption } = await generarBienvenida({ conn, userId, groupMetadata, chat })
    ...
    if (
      ...
      const { image, caption } = await generarDespedida({ conn, userId, groupMetadata, chat })
