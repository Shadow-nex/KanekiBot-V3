...
if (!i) i = {}
if (!i.download) i.download = ''

if (!i.download)
return conn.reply(m.chat, '⚠︎ No se pudo descargar este pin.', m)

let isVideo = i.download.includes(".mp4")
await conn.sendMessage(
m.chat,
{ [isVideo ? "video" : "image"]: { url: i.download }, caption: i.title || 'Pinterest' },
{ quoted: fkontak || m }
)
} else {
...
return {
title: result.title,
download: result.imageLargeUrl || result.images?.orig?.url || ''
}
}} catch {
return { msg: "Error, inténtalo de nuevo más tarde", download: '' }
}}
